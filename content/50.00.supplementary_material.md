...
In our study titled "Projecting genetic associations through gene expression patterns highlights disease etiology and drug mechanisms," we aimed to check the accuracy and reliability of our Gene-Level Statistics (GLS) model.
To do this, we looked at the error rates, specifically focusing on how often our model incorrectly flagged results as significant (with $p$-values below 0.05).
We also checked how well our model's predictions matched expected outcomes, a process known as calibration. 

For our analysis, we used data from the 1000 Genomes Project Phase III, focusing on 312 individuals of European descent.
We then created a hypothetical scenario by generating 1,000 traits that followed a normal distribution, meaning they had a set average and variance, to see how our model would perform under controlled conditions.

To conduct our analysis, we followed the standard procedures used in Transcriptome-Wide Association Studies (TWAS), specifically using two methods known as S-PrediXcan and S-MultiXcan.
These steps included: 1) carrying out a Genome-Wide Association Study (GWAS) using a method that assumes genes add up their effects in a straightforward way, 2) preparing the GWAS data through steps like harmonization and imputation as outlined in a referenced study, and 3) applying the S-PrediXcan and S-MultiXcan methods to analyze the data.
We provide further details on each of these steps below.

**Step 1 - GWAS (Genome-Wide Association Studies)**.
We carried out basic quality control steps, which included removing genetic variants that had more than 1% of data missing, those with a minor allele frequency (MAF) less than 1%, or with a minor allele count (MAC) below 20, and those not meeting the Hardy-Weinberg equilibrium (HWE) threshold of 1e-6.
Additionally, we excluded samples that showed significant discrepancies in reported sex or were closely related (up to second-degree relatives).
For our analysis, we factored in sex and the top 20 principal components to account for potential confounding variables.
This process was applied to 5,923,554 genetic variants across 1,000 randomly selected phenotypes (traits or characteristics).

**Step 2 - Processing Genetic Association Studies**.
This phase involves standardizing genetic association study data and estimating statistical scores (known as $z$-scores), essential steps in the TWAS (Transcriptome-Wide Association Studies) framework to match the genetic variations in our prediction models accurately.
The necessary scripts for these operations can be found online at [https://github.com/hakyimlab/summary-gwas-imputation].
We applied these methods to 1,000 randomly selected traits, resulting in a comprehensive dataset of 8,325,729 genetic variations, which includes both the original and the estimated statistical scores.
...
In our study titled "Projecting genetic associations through gene expression patterns highlights disease etiology and drug mechanisms," we explored how genes are connected to diseases and potential drug treatments.
To do this, we used a sophisticated method, referred to as the GLS model, to analyze the relationship between 987 genetic patterns (known as LVs) and the outcomes of 1,000 genetic studies on various diseases.
This involved creating a special gene correlation matrix for our specific group of study participants.

We then compared the results from our GLS model to those from a more basic method called the ordinary least squares (OLS) model, which doesn't account for the possibility that genes might be related to each other.
By comparing the two, we found that the GLS model was slightly better at accurately identifying true genetic associations, showing a lower average error rate than the OLS model.
This suggests that the GLS model is more reliable because it considers the complex relationships between genes.

Moreover, the GLS model was particularly effective at identifying groups of closely related genes that are important for understanding genetic associations with diseases.
For example, it successfully highlighted specific genetic patterns, such as LV234, LV847, LV45, and LV800, as being significant.
On the other hand, the simpler OLS model did not perform as well in these cases, showing higher error rates and less reliable results.

In summary, our comparison highlights the advantages of using advanced methods like the GLS model for genetic studies, as it provides a more accurate understanding of how genes relate to diseases and potential treatments.

In our study, we found some latent variables (LVs) that showed higher-than-expected error rates in both the Generalized Least Squares (GLS) and Ordinary Least Squares (OLS) models.
Interestingly, these LVs didn't necessarily have a large number of closely related genes.
For instance, LV914, as depicted in one of our figures, is a good example of this observation.
The reason behind these higher error rates could be due to inaccuracies in how well the MultiXcan model, which we used to analyze individual-level data, matches its summary-level counterpart (for more details, see the Methods section).
As a result, we identified LVs with an error rate above 0.07 (a total of 127 LVs) and decided not to include them in our main analysis to ensure accuracy.
When we applied our method to actual data, we did not observe these error rate issues, as shown in another figure.
...
In our study titled "Projecting genetic associations through gene expression patterns highlights disease etiology and drug mechanisms," we explored two main scenarios to understand how gene expressions relate to traits and potentially influence disease causes and drug actions.
Our keywords include genetic studies, functional genomics, gene co-expression, therapeutic targets, drug repurposing, and clustering of complex traits.

In the first scenario, we mixed up the genes for each trait in a data table (let's call it M), and then we used a special method to map this mixed-up data into a simplified model.
In the second scenario, we first mapped the original data table (M) into the simplified model and then mixed up some elements (called LVs) in the mapped data for each trait.

For both scenarios, we followed the same steps as we did with the actual data, which involved grouping similar traits together using a specific process (detailed in the Methods section under clustering).
We then combined the results from these groupings using a standard approach to get our final groupings of traits.

Lastly, we evaluated our work by 1) checking how consistent the groupings were when we used different methods, and 2) seeing how much the final groupings agreed with the combined results from all the methods we used.
...
In the supplementary section of our paper titled "Projecting genetic associations through gene expression patterns highlights disease etiology and drug mechanisms", we present additional analysis results.
Specifically, Figure @fig:sup:clustering:agreement illustrates the outcomes of testing two theoretical scenarios that did not involve actual data.
In these scenarios, we found almost no agreement between the grouped categories created by different methods and data interpretations.
This was anticipated and indicates that when using varied techniques and data forms, there's no common ground in how the categories are grouped.

However, when we applied the same grouping method to real data, a different picture emerged.
We were able to identify pairs of traits that consistently grouped together across various methods.
For a more detailed analysis, we focused on groupings that were in the top 25% in terms of agreement, which are depicted in the clustering tree shown in Figure @fig:clustering:tree.
This approach allowed us to better understand how certain traits are related, offering insights into the underlying biology and potential treatment targets.
