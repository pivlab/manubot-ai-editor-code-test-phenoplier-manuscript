...
We tested our GLS model for errors and accuracy by comparing $p$-values to a threshold of 0.05, using a null model with random traits and genotype data from 1000 Genomes Phase III.
We chose 312 individuals of European descent and examined 1,000 traits from a standard normal distribution $\mathcal{N}(0,1)$.
We followed the standard procedures for TWAS approaches (S-PrediXcan and S-MultiXcan), which included: 1) conducting a standard GWAS using linear regression with an additive genetic model, 2) performing various GWAS processing steps such as harmonization and imputation as described in a previous study, 3) carrying out S-PrediXcan and S-MultiXcan analyses.
...
We followed standard quality control procedures by filtering out variants with missing call rates above 0.01, minor allele frequencies below 1%, minor allele counts below 20, and Hardy-Weinberg equilibrium below 1e-6.
We also excluded samples with discrepancies in sex and high levels of relatedness (up to second degree).
Additionally, we included sex and the top 20 principal components as covariates in the analysis.
The association test was performed on a total of 5,923,554 variants across all 1,000 random phenotypes.

In Step 2 of the study, we processed GWAS data by harmonizing and imputing $z$-scores.
These steps are essential for the TWAS pipeline to have a sufficient overlap with SNPs in prediction models.
The scripts needed to perform these tasks can be found at https://github.com/hakyimlab/summary-gwas-imputation.
...
We then compared the results of our model with a standard least squares model that assumes genes are independent.
The graph shows that the model we used, called GLS, had a slightly lower average error rate (0.0558, SD=0.0127) compared to the standard model (0.0584, SD=0.0140).
The GLS model also produced $p$-values that were closer to what we would expect in a random distribution.
Importantly, the GLS model was able to correct for patterns where genes were closely related, like LV234, LV847, LV45, and LV800.
In contrast, the standard model had higher error rates and lower $p$-values in these cases.

We identified additional LVs that had higher-than-expected mean type I errors in both the GLS and OLS models, even though they did not have a relatively large number of adjacent genes at the top.
One such LV is LV914, which is illustrated in Figure 1.
...
As a result, we identified 127 LVs with a type I error rate larger than 0.07 and excluded them from our main analyses.
No signs of inflation were observed when applying the method to real data, as shown in Figure 2.
...
In the first scenario, we rearranged genes in matrix M for each trait and then projected this randomized matrix into a hidden space.
...
In both scenarios, we used the same clustering process as in the real data analysis, creating multiple partitions that were later merged using consensus functions to determine the final trait groupings.
Additionally, we calculated stability statistics for the partitions generated by different algorithms and assessed the agreement of the final consensus partition with the ensemble.
...
The analysis results (Figure 1) indicate that, in the simulated null scenarios, there is minimal agreement between the consensus partitions and the ensemble, suggesting no consensus among different clustering algorithms and data representations.
However, with real data, the consensus clustering method successfully groups trait pairs across the ensemble.
Only partitions above the 75th percentile were included in the main analyses and are displayed in the clustering tree (Figure 2).
